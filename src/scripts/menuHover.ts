document.addEventListener("DOMContentLoaded", () => {
  ...
  const links = document.querySelectorAll<HTMLAnchorElement>(
    "#mainNav a[data-menu-link]"
  );
  const desktopList = document.getElementById(
    "previewList"
  ...
  const normalizeKey = (rawKey: string | undefined, href: string): string => {
  ...
  const fillList = (ul: HTMLUListElement, items: string[]) => {
    ul.innerHTML = items.map((t) => `<li>${t}</li>`).join("");
  };
  const expand = (ul: HTMLUListElement) => {
    ul.classList.remove("opacity-0");
    ul.classList.add("opacity-100");
    ul.style.maxHeight = ul.scrollHeight + "px";
  };
  const collapse = (ul: HTMLUListElement) => {
    ul.classList.remove("opacity-100");
    ul.classList.add("opacity-0");
    ul.style.maxHeight = "0px";
    window.setTimeout(() => (ul.innerHTML = ""), 200);
  };
  const collapseAllExcept = (except?: HTMLUListElement) => {
    nav.querySelectorAll<HTMLUListElement>("ul[data-submenu]").forEach((u) => {
      if (u !== except) {
